...
program:
  | ds = decls EOF { let (vds, fds) = ds in Prog (vds, fds) }
;

decls:
  | { ([], []) }
  | fd = fun_decl ds = decls { let (vds,fds) = ds in (vds,fd :: fds) }
  | vd = var_decl ds = decls { let (vds,fds) = ds in (vd::vds, fds)}
;
...
fundecl_list:
  | { [] }
  | f = fun_decl fl = fundecl_list { f::fl }
;
