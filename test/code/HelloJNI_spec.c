...
#define JNI_Output_char_ptr(input,env) printf("Result:%x\n",*input)
#define JNI_Output_jstring(input,env) printf("Result:%x\n", *((*env)->GetStringChars(env, input, NULL)))
...
char *leaf_char_PTR_RisClassSigC(JNIEnv *env, const char *used_in, const char *refinement) {
    ...
    //for spec
    else if (strcmp(refinement, "@!isNULLCL@!isObjectCL") == 0) {
        return "java/lang/Boolean";
    }
    //for spec
    else if (strcmp(refinement, "") == 0) {
        return "java/lang/Number";
    }
    else {
        assert(false && "JNI_Test_Gen_Fail");
...
//for spec
jclass leaf_jclass_RisObjectCL(JNIEnv *env, const char *used_in, const char *refinement) {
    jclass class1 = (*env)->FindClass(env, "java/lang/Object");
...
jsize leaf_jsize(JNIEnv *env, const char *used_in, const char *refinement) {
    ...
    //for spec
    else if (starts_ends_with(used_in, "Get", "ArrayElement") || starts_ends_with(used_in, "Set", "ArrayElement")) {
        // GetXxxArrayElement, SetXxxArrayElement
        return 0;
    }
    else {
        assert(false && "JNI_Test_Gen_Fail");
...
jmethodID leaf_jmethodID_RisInitM(JNIEnv *env, const char *used_in, const char *refinement) {
    ...
    //for spec
    else if (contains(refinement, "@isSubClassO")) {
        jclass class1 = (*env)->FindClass(env, "Class1");
        assert(class1 != NULL);
...
jstring leaf_jstring(JNIEnv *env, const char *used_in, const char *refinement){
  ...
  //for spec
  else if(strcmp(refinement, "@isGetStringCharsJC@isUnicodeJC") == 0){
        jstring res = (*env)->NewString(env,"HSJTEST",6);
        assert(res != NULL && "JNI_Test_Gen_Fail");
        return res; 
  }
  //for spec
  else if(strcmp(refinement, "@isGetStringCriticalJC@isUnicodeJC") == 0){
        jstring res = (*env)->NewString(env,"HSJTEST",6);
        assert(res != NULL && "JNI_Test_Gen_Fail");
        return res; 
  }
  //for spec
  else if(strcmp(refinement, "isGetStringUTFCharC@isUTF8C") == 0){
        jstring res = (*env)->NewStringUTF(env,"HSJTEST");
        assert(res != NULL && "JNI_Test_Gen_Fail");
        return res; 
  }
...
jbyteArray leaf_jbyteArray(JNIEnv *env, const char *used_in, const char *refinement){
  if(strcmp(used_in, "ReleaseByteArrayElements") == 0){
    ...
    if (contains(refinement, "@!isInThisClassM") && (contains(refinement, "@isGetByteArrayElements"))) {
        return (*env)->GetByteArrayElements(env,((*env)->NewByteArray(env, 9)), NULL);
    }
    else{ return jbyteArrayGlobal; }
  }
...
//for spec
char *leaf_char_PTR_RisArrayClassSigC(JNIEnv *env, const char *used_in, const char *refinement){
        jstring tmp = (*env)->NewStringUTF(env,"[LClass1;");
        assert(tmp != NULL && "JNI_Test_Gen_Fail");
        char *res = (*env)->GetStringUTFChars(env, tmp, NULL);
        assert(res != NULL && "JNI_Test_Gen_Fail");
        return res; 
...
//for spec
jchar *leaf_jchar_PTR_RisUnicodeJC(JNIEnv *env, const char *used_in, const char *refinement){
        jstring tmp = (*env)->NewString(env,"BYTEDATA",7);
        assert(tmp != NULL && "JNI_Test_Gen_Fail");
        jchar *res = (*env)->GetStringChars(env, tmp, NULL);
        assert(res != NULL && "JNI_Test_Gen_Fail");
        return res; 
...
//for spec
jweak leaf_jweak(JNIEnv *env, const char *used_in, const char *refinement){
  jclass testClass = (*env)->FindClass(env, "Class1");
  assert(testClass != NULL && "JNI_Test_Gen_Fail");
  
  jobject tc_class = (*env)->NewWeakGlobalRef(env, testClass);
  assert(tc_class != NULL && "JNI_Test_Gen_Fail");
  return tc_class;
}
...
//for spec
jint leaf_jint_RisPositiveJI(JNIEnv *env, const char *used_in, const char *refinement){
 return 924;
}

//for spec
JavaVM *leaf_JavaVM_PTR(JNIEnv *env, const char *used_in, const char *refinement){
  return &jvm;
}

//for spec
jint leaf_jint_RisThisArrayLenJI(JNIEnv *env, const char *used_in, const char *refinement){
  return 1;
}

//for spec
jthrowable leaf_jthrowable(JNIEnv *env, const char *used_in, const char *refinement){
    jclass clazz = (*env)->FindClass(env, "java/lang/Throwable");
    assert(clazz != NULL && "JNI_Test_Gen_Fail");
...
jclass leaf_jclass_RisThrowableCL(JNIEnv *env, const char *used_in, const char *refinement){
    jclass clazz = (*env)->FindClass(env, "java/lang/Throwable");
    assert(clazz != NULL && "JNI_Test_Gen_Fail");
    return clazz;
}
...
//for spec
jmethodID leaf_jmethodID(JNIEnv *env, const char *used_in, const char *refinement){

 if (strcmp(refinement, "@isReflectConstructorO") == 0){
    jclass clazz = (*env)->FindClass(env, "Class1");
    assert(clazz != NULL && "JNI_Test_Gen_Fail");
    
    jmethodID methodID = (*env)->GetMethodID(env, clazz, "<init>", "(I)V");
    assert(methodID != NULL && "JNI_Test_Gen_Fail");
    return methodID;
 }
 else if (strcmp(refinement, "@isReflectMethodO") == 0){
    jclass clazz = (*env)->FindClass(env, "Class1");
    assert(clazz != NULL && "JNI_Test_Gen_Fail");
    ...
    jmethodID methodID = (*env)->GetMethodID(env, clazz, "publicCharMethod1", "(I)C");
    assert(methodID != NULL && "JNI_Test_Gen_Fail");
    return methodID;
 }
...
jboolean *leaf_jboolean_PTR_RisThisBooleanArray_RNot_isGetBooleanArrayElements_RNot_isNULLJB(JNIEnv *env, const char *used_in, const char *refinement){
 res = (*env)->GetBooleanArrayElements(env,(*env)->NewBooleanArray(env, ARRAY_SIZE),NULL);
 assert(res != NULL && "JNI_Test_Gen_Fail");
 return res;
}
...
//for spec
jfieldID leaf_jfieldID(JNIEnv *env, const char *used_in, const char *refinement){
 jfieldID fID;
 jclass testClass = (*env)->FindClass(env, "Class1");
 assert(testClass != NULL && "JNI_Test_Gen_Fail");
 
 fID = (*env)->GetStaticFieldID(env, testClass, "publicStaticIntField1", "I");
 assert(fID != NULL && "JNI_Test_Gen_Fail");
...
//for spec
jobject leaf_jobject_RisGlobalO(JNIEnv *env, const char *used_in, const char *refinement){
  jclass testClass = (*env)->FindClass(env, "Class1");
  assert(testClass != NULL && "JNI_Test_Gen_Fail");
  jobject res_obj = (*env)->NewGlobalRef(env, testClass);
  assert(res_obj != NULL && "JNI_Test_Gen_Fail");
  return res_obj;
}
...
jobjectArray leaf_jobjectArray(JNIEnv *env, const char *used_in, const char *refinement){
  ...
  */
  ...
  jobject tc_class = (*env)->NewGlobalRef(env, testClass);

  jmethodID tc_constructor = (*env)->GetMethodID(env, tc_class, "<init>", "(I)V"); 
  jobject tcObject = (*env)->NewObject(env, tc_class, tc_constructor);

  jobjectArray res = (*env)->NewObjectArray(env, 1, testClass, tcObject);
  assert(res != NULL && "JNI_Test_Gen_Fail");
  return res; 
}
...
//for spec
jweak leaf_jweak_RNot_isDeletedJW(JNIEnv *env, const char *used_in, const char *refinement){
  jclass localRefCls = (*env)->FindClass(env, "java/lang/String");
  if (localRefCls == NULL) {
       assert(false && "JNI_Test_Gen_Fail");
       return NULL; /* exception thrown */
  }
  jweak res_obj = (*env)->NewWeakGlobalRef(env, localRefCls);
  assert(res_obj != NULL && "JNI_Test_Gen_Fail");
  return res_obj;
}
